      ...
      <el-aside width="340px" class="bg-color">
        <el-container class="h bg-color">
          <el-header height="50px" class="nav">
            <div
              class="nav-box"
              @click="pathTypeChange('ETS2')"
              :class="{ active: pathType === 'ETS2' }"
            >
              <div class="img">
                <i class="iconfont iconkache3" style="margin-right: 10px"></i>ETS2
              </div>
            </div>
            <div
              class="nav-box"
              @click="pathTypeChange('ATS')"
              :class="{ active: pathType === 'ATS' }"
            >
              <div class="img">
                <i class="iconfont iconhuoche" style="margin-right: 10px"></i>ATS
              </div>
            </div>
          </el-header>
          <el-main class="aside">
            <div class="icon_box">
              <i
                class="iconfont iconlogo-640 ets666_icon cursor_pointer"
                @click="goToWeb('https://ets666.com/')"
              ></i>
              <div class="websize cursor_pointer" @click="goToWeb('https://ets666.com/')">
                ETS666.COM
              </div>
              <div class="line"></div>
            </div>
...
const init = async () => {
  ...
  const dir = await ipc.invoke("mapDirName", {
    dir: props.savePath,
    filedirname: "/profiles",
  });
  try {
  ...
  try {
    let res: RandomJob[] | null = null;
    if (pathType.value === "ETS2") {
      res = await ipc.invoke("ets666", "random_jobs");
    } else if (pathType.value === "ATS") {
      res = await ipc.invoke("ets666", "random_jobs_ats");
    }
    if (res) {
...
const changeProfile = async (path: string) => {
  save.value = "";
  saveOptions.value = [];
  const file = await ipc.invoke("mapDirName", {
    dir: props.savePath,
    filedirname: `/profiles/${path}/save`,
  });
...
const saveSetting = async () => {
  ...
  if (
    ...
    const res = await ipc.invoke("SiiDecrypt", {
      dir: gameSiiPath,
